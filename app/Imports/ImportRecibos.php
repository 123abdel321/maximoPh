...
class ImportRecibos implements
    ToCollection,
    WithHeadingRow,
    WithChunkReading,
    WithBatchInserts,
    ...
    // WithEvents
{
    ...
    protected $comprobanteRecibosCaja;
    ...
    public function collection(Collection $rows)
    {
        ...
        event(new PrivateMessageEvent("importador-recibos-" . $this->url_notification, [
            'name' => 'progress',
            'tipo' => 'info',
            'mensaje' => "Iniciando procesamiento de {$this->totalValidRows} registros...",
            'titulo' => 'Importación de pagos',
            'progress' => 0,
            'processed' => 0,
    ...
    private function inicializarConfiguracion()
    {
        ...
        $this->idComprobanteRecibosCaja = optional(Entorno::where('nombre', 'id_comprobante_recibos_caja')->first())->valor ?? null;
        $this->comprobanteRecibosCaja = $this->idComprobanteRecibosCaja ? Comprobantes::where('id', $this->idComprobanteRecibosCaja)->first() : null;
    }
    ...
    private function procesarFila($row): array
    {
        ...
        $comprobanteData = $this->comprobanteRecibosCaja;
        if ($row['cod_comprobante']) {
            $comprobanteSearch = Comprobantes::where('codigo', $row['cod_comprobante'])->first();
            if ($comprobanteSearch) {
                $comprobanteData = $comprobanteSearch;
            } else {
                $comprobanteData = null;
                $estado = 1;
                $observacion .= "El código del comprobante: {$row['cod_comprobante']}, no existe!<br>";
            }
        }
        ...
        return [
            ...
            'id_comprobante' => $comprobanteData ? $comprobanteData->id : null,
            'nombre_comprobante' => $comprobanteData ? $comprobanteData->codigo. ' - '. $comprobanteData->nombre : null,
        ...
        return [
            AfterImport::class => function(AfterImport $event) {
                ...
                event(new PrivateMessageEvent("importador-recibos-" . $this->url_notification, [
                    'name' => 'carga',
                    'tipo' => 'exito',
                    'mensaje' => 'Carga de plantilla de recibos finalizado totalmente!',
                    'titulo' => 'Plantilla de recibos',
                    'autoclose' => false
                ]));
            },
        ];
